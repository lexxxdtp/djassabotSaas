...
> - ✅ **Migrations appliquées sur Supabase le 14 septembre** et vérifiées par requête : `lock_product_images_storage.sql` (aucune écriture anonyme sur les photos, 5 Mo, types image, `adjust_stock` réservé au `service_role`) et `add_place_order_rpc.sql` (recette PGlite 15/15, fonctions réservées au `service_role`). Code poussé sur `main`, frontend Vercel en ligne ; le backend sera à jour au redémarrage du VPS (`git pull`, build, `pm2 restart`).
