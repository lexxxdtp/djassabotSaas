...
> **État courant — 20 septembre 2026 :** une seule copie locale, `~/djassabotSaas`. Backend déployé sur le VPS (`20c0db0`, PM2 stable, API en HTTPS) et frontend Vercel à jour. `ALLOWED_ORIGINS` du VPS porte désormais l'origine `https://187-77-171-44.nip.io` (sortie du code).
...
> - **Refonte terminée dans les recoins** (20 septembre, second passage) : les tiroirs de réglages gardaient l'ancienne palette — interrupteur de livraison orange, pastilles jaunes et bleues, boutons émeraude, gris `zinc`/`neutral` de Tailwind, et un reste de thème indigo sur le moyen de paiement choisi. Tout passe aux jetons et à l'accent `#00D97E`. Les seules couleurs non vertes qui restent sont porteuses de sens : états de commande et de conversation, force du mot de passe, aperçu WhatsApp (qui imite WhatsApp).
> - 🐞 **Défauts de mise en page trouvés dans la foulée** : à 320 px, les zones de livraison et les sept lignes d'horaires débordaient de leur carte (une ligne d'horaires réclamait 440 px pour 230 disponibles) ; « Professionnel » dépassait de sa vignette. La recette ne les voyait pas : elle balayait les pages sans jamais **ouvrir** les tiroirs, et son détecteur ignorait tout ce qui vit dans un conteneur défilant — or un tiroir défile verticalement. Elle mesure désormais, tiroir par tiroir, le contenu plus large que sa boîte (vérifié : le test échoue bien si on réintroduit le défaut).
...
> - **Lot 4 — compte, récupération et identité** (174 tests backend) :
>   - **Jetons hachés** (M2) : `auth_tokens.token_value` contient désormais une empreinte SHA-256, plus le code à six chiffres ni le lien de réinitialisation en clair. Les jetons précédents du même identifiant sont effacés à chaque demande (cinq demandes ne laissent plus cinq codes valables). Tolérance datée : la vérification accepte encore les anciens jetons en clair, à retirer dès que les liens d'avant le 20 septembre ont expiré (une heure).
>   - **« Mot de passe oublié » ne ment plus** (A14) : réponse strictement identique que l'adresse existe ou non (avant, deux phrases différentes révélaient qui a un compte), et un refus de Resend devient un incident journalisé `alerte: RESET_EMAIL_NON_ENVOYE` au lieu d'un « lien envoyé » mensonger. La route `verify-phone-reset` est **supprimée** : elle promettait un SMS que rien n'envoyait et fabriquait un jeton de réinitialisation pour n'importe quel numéro, sans authentification.
>   - **Changer de mot de passe ferme les sessions** (A21/F1) : le JWT porte une empreinte du mot de passe (`utils/sessionFreshness.ts`), revérifiée à chaque requête avec un cache de 10 s. Un jeton volé ne survit plus à une réinitialisation. Les jetons émis avant cette version restent tolérés jusqu'à leur expiration (7 jours).
>   - **Campagnes sous verrou** (M3, A17) : une seule campagne à la fois par boutique et `BROADCAST_DAILY_LIMIT` (3) par jour — deux envois simultanés divisaient par deux le délai anti-ban, c'est-à-dire le meilleur moyen de faire **bannir le numéro WhatsApp**. Le journal d'activité utilise `type: 'action'` (le type `campaign` était refusé par la contrainte : aucune campagne n'avait jamais été tracée).
>   - Durcissements : `reset-password` passe sous limitation de débit, l'origine `nip.io` n'est plus codée en dur (`ALLOWED_ORIGINS`), le logger masque `authorization`, `password`, `token`, `token_value`… **F5 reste ouvert volontairement** : les 9 alertes npm viennent toutes de `uuid < 11.1.1`, dépendance transitive de `google-gax`/`node-cron`, corrigeable uniquement par `npm audit fix --force` qui casserait des versions majeures. Faille non atteignable ici (v3/v5/v6 avec `buf`, non utilisés).
> - ✅ **L'OTP téléphone Firebase fonctionne** (constaté le 20 septembre : une vendeuse a reçu son SMS et l'a validé). Le blocage `auth/error-code:-39` décrit en §6bis.0 appartient au passé ; le projet Firebase est `djassabot-prod`.
> - ✅ **Resend délivre vers des adresses tierces** : le lien de réinitialisation envoyé à `fatakeita@gmail.com` a été accepté par le service (journal du 20 septembre, 14 h 33 UTC). La note de juin « les emails ne partent que vers anadorbreak@gmail.com » est périmée.
> - 🐞 **Récupération par téléphone corrigée** : `forgotPasswordPhone` traitait la branche « cet utilisateur a un email » AVANT de vérifier le jeton Firebase. Le code SMS ne servait donc à rien et la vendeuse était renvoyée vers une adresse qu'elle n'ouvre pas (compte créé avec l'email d'une autre personne). Un code vérifié ouvre maintenant directement la réinitialisation, et l'écran n'annonce plus un email qui n'est pas parti.
> - **Choix du modèle IA tranché** (étude du 20 septembre, prix vérifiés chez les fournisseurs) : **on reste sur Gemini**, à cause des vocaux. L'audio entre nativement dans Gemini (0,30 $/M sur Flash-Lite) alors qu'OpenAI et Claude imposent une transcription séparée à 0,017 $/minute — **28 fois plus cher** pour un vocal de 30 s, et il faudrait trois fournisseurs pour couvrir texte + audio + image.
>   - **Routage à deux modèles** (`services/ai/choixModele.ts`) : `gemini-2.5-flash-lite` pour le trafic courant, `gemini-2.5-flash` quand une erreur coûte de l'argent — reçu de paiement, négociation avec prix plancher caché, message précédant l'écriture d'une commande. Coût mesuré : 0,25 FCFA le message courant contre 0,88 avant.
>   - **Réflexion facturée au prix de la sortie** : 0 en routine, 512 sur les appels délicats. `GEMINI_MAX_OUTPUT_TOKENS` passe de 2048 à **512** (une réponse WhatsApp fait deux phrases) et l'historique envoyé de 20 à **10 tours** (`AI_HISTORY_TURNS`).
>   - **Retour en arrière en une commande** : `GEMINI_MODEL=gemini-2.5-flash` désactive le routage et impose ce modèle partout. Chaque étage reste remplaçable (`GEMINI_MODEL_ROUTINE`, `GEMINI_MODEL_DELICAT`).
>   - **Mesure** : `recordAiUsage` journalise déjà les jetons d'entrée, de sortie et de réflexion ; s'y ajoutent le **modèle utilisé** et les **jetons servis par le cache implicite** de Gemini (actif d'office au-delà de 2 048 jetons de préfixe stable — le prompt système et le catalogue sont déjà placés en tête, ne pas changer cet ordre ni celui du catalogue sous peine de perdre la remise en silence).
>   - ⚠️ **Non vérifié en production** : les crédits prépayés Gemini sont épuisés (HTTP 402). Aucun appel réel n'a pu confirmer la qualité de Flash-Lite en nouchi. Protocole avant d'ouvrir à des vendeurs : 50 conversations réelles au simulateur sur les deux modèles, avec trois critères non négociables — aucun prix engagé hors règles, aucun paiement validé sur simple image, aucune commande sans accord explicite.
...
> - ✅ **Migrations appliquées sur Supabase le 14 septembre** et vérifiées par requête : `lock_product_images_storage.sql` (aucune écriture anonyme sur les photos, 5 Mo, types image, `adjust_stock` réservé au `service_role`) et `add_place_order_rpc.sql` (recette PGlite 15/15, fonctions réservées au `service_role`). Code déployé : frontend Vercel en ligne et **backend à jour sur le VPS depuis le 20 septembre** (`1ab30e2`, PM2 stable au démarrage automatique, Supabase et Firebase initialisés). ⚠️ **Aucune session WhatsApp active** : les dossiers `backend/auth_info_baileys/*` sont vides depuis août (les sauvegardes de `/home/alex/backups/` ne contiennent que des dossiers vides, 299 octets). Chaque boutique doit être réappairée depuis Réglages → WhatsApp (code de jumelage) ; au démarrage, `startAllTenantInstances` n'ouvre une session que pour les tenants `whatsapp_connected = true`.
> - Restent ouverts : lots 3 à 5 de l'audit (paiements et abonnements A08–A12, compte A14–A21, exploitation A23–A24), le réappairage WhatsApp, et l'[audit de sécurité du 18 septembre](AUDIT_SECURITE_2026-09-18.md) (E2 isolation, M2 jetons en clair, M3 verrou campagnes, M4 quotas IA en mémoire).
