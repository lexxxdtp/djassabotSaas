...
- [~] **Clavier et focus des modales faits le 12 septembre ; le reste de la ligne est ouvert.** Aucune modale ne déclarait `role="dialog"`, ne se fermait avec Échap, ni ne retenait le focus : un vendeur au clavier tabulait derrière la modale sans le voir et modifiait la page cachée dessous, et un lecteur d'écran ne savait pas qu'une fenêtre s'était ouverte. Le hook `useModalA11y` (Échap, piège de tabulation circulaire, focus rendu à l'élément d'origine) est appliqué aux **sept** modales du projet : détail de commande, formulaire produit, compte, tiroir de réglages, suppression de produit (deux écrans), confirmation d'envoi de campagne. Cette dernière ignore Échap pendant un envoi en cours.

  **Complété le 13 septembre sur les PAGES PUBLIQUES, vérifié au navigateur** (Chromium à 320 et 390 px, chaque défaut mesuré avant/après — page de garde, connexion, inscription, mots de passe, pages légales) :
  - **Petit écran 320 px** : le bloc de texte du hero mesurait 480 px sur un écran de 320 et était tronqué au bord droit (le ® de la marque, le point final du titre, le « À » de la description). La cause n'était pas la taille de police mais la grille : « Même sans vous. » est en `white-space: nowrap`, et une piste `1fr` se dimensionne sur le contenu minimal de ses enfants. Corrigé par `minmax(0, 1fr)` plus un palier de styles sous 400 px, qui manquait — le dernier s'arrêtait à 540 px.
  - **Zoom interdit** : `maximum-scale=1.0, user-scalable=no` empêchait le zoom à deux doigts, donc toute vendeuse presbyte d'agrandir son tableau de bord. Retiré. La crainte du zoom iOS au focus est infondée ici : tous les champs héritent déjà de 16 px (mesuré, pas supposé).
  - **Contraste** : trois textes à `#555` mesuraient 2,5 à 2,8 pour un minimum AA de 4,5. Passés à `#888` (5,9), déjà dans le design system. Zéro échec restant sur les pages publiques.
  - **Cibles tactiles** : de 13 à 6 sur la page de garde, de 3 à 1 sur la connexion. La case « Me garder connecté » mesurait 16×16, sans nom accessible, son libellé relié par un simple `onClick` : case et texte ne forment plus qu'une cible, annoncée comme case à cocher.
  - **Langue** : `lang="en"` sur une application entièrement française — un lecteur d'écran prononçait le français avec la phonétique anglaise. Passé à `fr`.
  - **Titres d'onglet** : toutes les routes partageaient le même `<title>`. Hook `usePageTitle` ajouté, plus une meta description qui manquait.

  ⚠️ **Décision de design à trancher, volontairement non prise** : `#555` apparaît 118 fois et `#444` 7 fois dans `frontend/src`. Les deux échouent au contraste AA sur fond noir (2,82 et 1,9). Les remplacer partout modifierait la hiérarchie visuelle de toute l'application, et le design system du CLAUDE.md les définit explicitement comme tokens. Seuls les cas vérifiés au navigateur ont été corrigés. Si Alex veut généraliser : `#888` est la valeur sûre, `#767676` étant le seuil exact sur noir pur.

  **Restent ouverts** : TOUT le dashboard, non vérifiable sans backend allumé (ces pages exigent une connexion). Plus : mouvement réduit hors page de garde, clavier virtuel, et les cibles tactiles résiduelles entre 35 et 37 px.
