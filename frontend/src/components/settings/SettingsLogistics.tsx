        ...
        <div className="space-y-6">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="sr-only"
                            />
                            <div className={`w-11 h-6 rounded-full transition-colors ${config.deliveryEnabled ? 'bg-[#00D97E]' : 'bg-white/15'}`}>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {(Array.isArray(config.deliveryZones) ? config.deliveryZones : []).map((zone, index) => (
                                // Sur un écran de 320 px, le nom et le prix côte à côte
                                // débordaient de la carte : le champ FCFA sortait de
                                // l'écran. Ils s'empilent tant que la place manque.
                                <div key={index} className="flex flex-col sm:flex-row sm:items-center gap-3 bg-white/5 rounded-lg p-3">
                                    <input
                                        type="text"
                                        value={zone.name}
                                        onChange={(e) => {
                                        ...
                                        className="w-full sm:flex-1 min-w-0 bg-white/10 border border-[var(--color-border)] rounded-lg p-2 text-white text-sm placeholder:text-[var(--color-muted)]"
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="relative flex-1 sm:flex-none">
                                            <input
                                                ...
                                                className="w-full sm:w-28 bg-white/10 border border-[var(--color-border)] rounded-lg p-2 pr-12 text-white text-sm"
                                                placeholder="1500"
                                            />
                                            <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[var(--color-muted)] text-xs">FCFA</span>
                                        </div>
                                        {(Array.isArray(config.deliveryZones) ? config.deliveryZones : []).length > 1 && (
                                            <button
                                                ...
                                                className="p-2 shrink-0 text-red-400 hover:bg-red-500/10 rounded-lg transition-colors"
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 text-[#00D97E] hover:text-white text-sm font-medium transition-colors"
                        >
                            ...
                            <div className="flex items-center gap-3">
                                <div className="relative flex-1 max-w-xs">
                                    <input
                                        ...
                                        className="w-full bg-white/5 border border-[var(--color-border)] rounded-lg p-3 pr-16 text-white focus:border-[#00D97E] outline-none"
                                        placeholder="50000"
                                    />
                                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-muted)] text-sm">FCFA</span>
                                </div>
                                <p className="text-xs text-[var(--color-muted)]">Livraison offerte à partir de ce montant (0 = jamais gratuit)</p>
                            </div>
                        </div>
                    </div>
                ) : (
            ...
            <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5 md:p-8">
                <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2 uppercase tracking-wider text-xs">
                    <span className="w-2 h-2 rounded-full bg-[#00D97E]"></span> Paiement
                </h2>
                <div className="flex flex-wrap gap-3">
                    {[
                        ...
                        return (
                            <label
                                key={method.id}
                                className={`flex items-center gap-2 bg-white/5 border rounded-lg px-4 py-3 cursor-pointer select-none transition-all ${isChecked ? 'border-[#00D97E]' : 'border-[var(--color-border)] hover:border-[var(--color-border)]'
                                    }`}
                            >
                                <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${isChecked ? 'border-[#00D97E] bg-[#00D97E]' : 'border-[var(--color-border)]'}`}>
            ...
            <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5 md:p-8 relative">
                <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2 uppercase tracking-wider text-xs relative z-10">
                    <span className="w-2 h-2 rounded-full bg-[#00D97E]"></span> Réception des Paiements
                </h2>

                <div className="relative z-10">
                    <p className="text-[var(--color-muted)] text-sm mb-6 max-w-2xl text-justify">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-6">
                        <div>
                            <label className="block text-xs font-semibold text-[var(--color-muted)] mb-2 uppercase tracking-wide">Opérateur / Banque</label>
                            <select
                                value={config.settlementBank || ''}
                                onChange={e => setConfig({ ...config, settlementBank: e.target.value })}
                                className="w-full bg-white/5 border border-[var(--color-border)] rounded-lg p-3 text-white focus:border-[#00D97E] outline-none appearance-none"
                                disabled={!!paystackSubaccountCode}
                            >
                                <option value="">Choisir un opérateur</option>
                                <option value="MTN" className="bg-black text-white">MTN Mobile Money</option>
                                <option value="Orange Money" className="bg-black text-white">Orange Money</option>
                                <option value="Wave" className="bg-black text-white">Wave</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-[var(--color-muted)] mb-2 uppercase tracking-wide">Numéro de Compte / Téléphone</label>
                            <input
                                type="text"
                                value={config.settlementAccount || ''}
                                onChange={e => setConfig({ ...config, settlementAccount: e.target.value })}
                                className="w-full bg-white/5 border border-[var(--color-border)] rounded-lg p-3 text-white focus:border-[#00D97E] outline-none font-mono tracking-wide placeholder:text-[var(--color-muted)]"
                    ...
                    {paystackSubaccountCode ? (
                        <div className="bg-[#00D97E]/10 border border-[#00D97E]/20 rounded-lg p-4 flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-[#00D97E] flex items-center justify-center shrink-0">
                                <CheckCircle className="text-black w-5 h-5" />
                            </div>
                            <div>
                                <div className="font-bold text-[#00D97E] text-sm">Compte de paiement actif</div>
                                <div className="text-xs text-[#00D97E]/70 font-mono">ID: {paystackSubaccountCode}</div>
                            </div>
                        </div>
                    ) : (
                        <button
                            ...
                            className="w-full py-3 bg-[#00D97E] hover:bg-[#27e996] disabled:opacity-50 disabled:cursor-not-allowed text-black font-bold rounded-lg transition-colors"
                        >
