        ...
        <div className="border-t border-[var(--color-border)] pt-4 mt-4">
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-semibold text-[var(--color-muted)] uppercase tracking-wide">Déclinaisons</span>
                    ...
                    <span className={`text-xs ${enabled ? 'text-[#00D97E]' : 'text-[var(--color-muted)]'}`}>
                        {enabled ? 'ON' : 'OFF'}
                    </span>
                </div>
            ...
            {!enabled ? null : variations.length === 0 ? (
                <p className="text-[var(--color-muted)] text-xs py-2 text-center border border-dashed border-[var(--color-border)] rounded-lg">Cliquez sur "Ajouter" pour créer une déclinaison</p>
            ) : (
                <div className="space-y-3">
                    {variations.map((variation, varIdx) => (
                        <div key={varIdx} className="bg-black/40 border border-[var(--color-border)] rounded-lg p-3">
                                ...
                                {variation.isCustom ? (
                                    <div className="flex-1 flex gap-2">
                                        <input
                                            ...
                                            className="flex-1 bg-white/5 border border-[var(--color-border)] rounded px-2 py-1.5 text-white text-sm focus:border-[#00D97E] outline-none"
                                        />
                                        <button
                                            type="button"
                                            onClick={() => {
                                    ...
                                    <div className="flex-1 flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                                        {templates.map(t => t.name).concat(['Taille', 'Couleur', 'Format']).filter((v, i, a) => a.indexOf(v) === i).map((optName) => (
                                            <button
                                                ...
                                                className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-colors border ${variation.name === optName
                                                    ...
                                                    : 'bg-[var(--color-surface)] text-[var(--color-muted)] border-[var(--color-border)] hover:bg-[#1a1a1a]'
                                                    }`}
                                            >
                                                {optName}
                                            </button>
                                        ...
                                        <button
                                            ...
                                            className="px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap bg-white/5 text-[var(--color-muted)] border border-[var(--color-border)] hover:bg-[#1a1a1a] hover:text-white"
                                        >
                                            + Autre
                                        </button>
                                    </div>
                            ...
                            {/* Options List */}
                            <div className="space-y-2 pl-2 border-l border-[var(--color-border)] ml-1">
                                {variation.options.map((opt, optIdx) => (
                                    <div key={optIdx} className="group relative bg-white/5 rounded p-2 flex items-start gap-3">
                                        ...
                                        <label className="relative w-10 h-10 flex-shrink-0 bg-black/30 rounded border border-[var(--color-border)] flex items-center justify-center cursor-pointer hover:border-[#00D97E] overflow-hidden">
                                            {uploading ? (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline">
                                                ...
                                                <span className="text-xs bg-white/10 text-[var(--color-muted)] px-1.5 py-0.5 rounded ml-2">
                                                    Stock: {opt.stock ?? 'Inf'}
                                                </span>
                                            </div>
                                            {opt.priceModifier !== 0 && (
                                        ...
                                        <button
                                            ...
                                            className="opacity-0 group-hover:opacity-100 absolute top-1 right-1 text-[var(--color-muted)] hover:text-red-500 bg-black/50 rounded-full p-0.5"
                                        >
                                            <X size={12} />
                                        </button>
                                    </div>
                                ...
                                <div className="flex gap-2 items-center mt-2">
                                    <input
                                        ...
                                        className="flex-1 w-full bg-white/5 border border-[var(--color-border)] rounded px-2 py-1 text-xs text-white focus:border-[#00D97E] outline-none"
                                    ...
                                    <input
                                        ...
                                        className="w-16 bg-white/5 border border-[var(--color-border)] rounded px-2 py-1 text-xs text-white focus:border-[#00D97E] outline-none"
                                        onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddOption(varIdx))}
                                    />
                                    <input
                                        type="number"
                                        ...
                                        className="w-14 bg-white/5 border border-[var(--color-border)] rounded px-2 py-1 text-xs text-white focus:border-[#00D97E] outline-none"
                                        onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddOption(varIdx))}
                                    />
                                    <button
                                        type="button"
