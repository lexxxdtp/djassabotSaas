    ...
    return (
        <div
            className={`relative border rounded-xl p-4 transition-all cursor-pointer hover:border-[#00D97E]/30 ${isCurrent
                ? 'bg-[var(--color-surface)] border-emerald-500 ring-1 ring-emerald-500'
                : recommended
        ...
        >
            {isCurrent && (
                <div className="absolute top-2 right-2 flex items-center gap-1 text-xs font-bold text-emerald-500 bg-emerald-950/30 px-2 py-0.5 rounded-full border border-emerald-500/20">
                    <Check size={10} /> ACTUEL
                </div>
            )}
            {recommended && !isCurrent && (
                <div className="absolute top-2 right-2 text-xs font-bold text-[#00D97E] bg-[#00D97E]/10 px-2 py-0.5 rounded-full border border-[#00D97E]/20">
                    POPULAIRE
                </div>
            )}
            ...
            <div className="mt-1 mb-3">
                ...
                <span className="text-xs text-[var(--color-muted)]">/mois</span>
            </div>
            ...
            <div
                ...
                <div className="md:w-64 bg-black border-r border-zinc-900 p-6 flex flex-col gap-2">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${activeTab === 'subscription' ? 'bg-[#00D97E]/10 text-[#00D97E]' : 'text-[var(--color-muted)] hover:text-white hover:bg-[var(--color-surface)]'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${activeTab === 'profile' ? 'bg-[#00D97E]/10 text-[#00D97E]' : 'text-[var(--color-muted)] hover:text-white hover:bg-[var(--color-surface)]'
                            }`}
                    >
                    ...
                    {activeTab === 'subscription' && (
                        <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                            <div>
                                ...
                                <p className="text-[var(--color-muted)] text-sm mt-1">Changez de plan pour débloquer plus de puissance.</p>
                            </div>
                            ...
                            <div className="bg-zinc-900/50 rounded-lg p-4 border border-zinc-800 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[var(--color-muted)] text-xs">Vos transactions sont chiffrées et sécurisées via Wave/OM.</p>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'profile' && (
                        <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                            <div>
                                ...
                                <p className="text-[var(--color-muted)] text-sm mt-1">Mettez à jour vos informations de contact.</p>
                            </div>

                            <div className="space-y-4 max-w-md">
                                <div>
                                    <label className="block text-xs font-bold text-[var(--color-muted)] mb-1 uppercase">Nom Complet</label>
                                    <div className="relative">
                                        <User size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-muted)]" />
                                        <input type="text" defaultValue={user?.full_name} className="w-full bg-black border border-[var(--color-border)] rounded p-2.5 pl-9 text-white text-sm focus:border-[#00D97E] outline-none" />
                                    </div>
                                </div>
                                <div>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-[var(--color-muted)] mb-1 uppercase">Nom du Business</label>
                                    <div className="relative">
                                        <Building size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-muted)]" />
                                        <input type="text" defaultValue={tenant?.name} className="w-full bg-black border border-[var(--color-border)] rounded p-2.5 pl-9 text-white text-sm focus:border-[#00D97E] outline-none" />
                                    </div>
                                </div>
