...
Relecture de `claude/dois-commiter-3h9adk` (`01aceab`) : 121 tests initiaux réussis, mais quatre risques restaient ouverts. Le correctif `codex/secure-payments` (`6650823`) recharge le montant et le contenu depuis la commande du tenant, refuse d'acquitter une commande non mise à jour, ajoute un registre SQL atomique des webhooks et une référence unique d'abonnement, protège la vérification inter-tenant et distingue livraison offerte de tarif inconnu. Ces changements et la documentation ont ensuite été fusionnés dans `main`. Validation du résultat : 127 tests backend, TypeScript backend, build et ESLint frontend réussis. Le projet Supabase a été restauré et les migrations stock, anti-doublon, Paystack puis durcissement des permissions ont été appliquées et vérifiées. Le contrôle confirme les nouvelles permissions Paystack, mais rappelle que la RLS globale historique reste à traiter avec test immédiat après redémarrage du VPS. Aucun déploiement VPS.
